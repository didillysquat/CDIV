...
We processed the 18S data using the cdiv.nf nextflow pipeline hosted [here](https://github.com/didillysquat/CDIV).

We output an unaligned fasta of the 198 unique most abundant
Cnidarian-derived 18S sequences and meta data using the script questions.py also available in the above GitHub repository.

We aligned the sequences using mafft:
```mafft --auto 18S.maj.seqs.fasta > 18S.maj.seqs.aligned.fasta```

We created the tree using iqtree with:
```iqtree -s 18S.maj.seqs.aligned.fasta -m SYM+I+G4 -b 100 -nt AUTO```

We will be creating a list of 'taxonomy ID groups'.
We will populate as we work through the tree examining the groupings.
```{r}
...
It is clear that Monitpora and Astreopora are good.
They form distinct monophyletic clades.

Go group by group to see if all annotations are found in the given group
and whether there are sample in the group that are not annotated as such
...
ggtree(ggtree.18S, aes(color=genus_alveopora)) +
    xlim(0, 4) + geom_tiplab() + theme_tree2()
```
OK. There is only the 1 g_Alveopora so we will need to verify this sample.
...
```{r}
id.group.tab.cols = c("tax_group_ID",	"number_of_samples",	"sample_list",	"number_unique_maj_cnid_18S_seqs_in_group",	"maj_cnid_18S_seq_list_seq",	"maj_cnid_18S_seq_list_names",	"has_genus_annotation",	"genus_annotation",	"evidence_for_tax_ID_group",	"has_been_visually_IDed",	"number_of_samples_visually_IDed",	"sample_list_visually_IDed",	"do_visual_IDs_agree")
...
for (seq.df.key in names(tax.group.ID.list)){
  ...
  tax.group.ID.data.list[[seq.df.key]] = c(
    ...
    "number_unique_maj_cnid_18S_seqs_in_group" = number_unique_maj_18S_seqs_in_group, "maj_cnid_18S_seq_list_seq" = maj_18S_seq_list_seq,
    "maj_cnid_18S_seq_list_names" = maj_18S_seq_list_names, "has_genus_annotation" = has_genus_annotation, "genus_annotation" = genus_annotation,
    "evidence_for_tax_ID_group" = evidence_for_tax_ID_group,"picture_urls" = "", "has_been_visually_IDed" = has_been_visually_IDed,
    "number_of_samples_visually_IDed" = number_of_samples_visually_IDed, "sample_list_visually_IDed" = sample_list_visually_IDed,
    "do_visual_IDs_agree" = visual_IDs_are_identical
    )
}
...
id.group.df$number_unique_maj_cnid_18S_seqs_in_group = as.numeric(id.group.df$number_unique_maj_cnid_18S_seqs_in_group)
...
id.group.df$do_visual_IDs_agree = as.logical(id.group.df$do_visual_IDs_agree)
```
Finally write out the df for upload into Python to produce the remaining tables
```{r}
write.csv(id.group.df, "tax.id.groups.csv")
```
